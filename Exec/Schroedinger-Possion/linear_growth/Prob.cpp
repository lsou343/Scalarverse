...
void AxSCH::prob_initdata_pos(
    const int i, const int j, const int k, Array4<amrex::Real> const &fields,
    GeometryData const &geomdata,
    const GpuArray<amrex::Real, BaseAx::max_prob_param> &prob_param) {
  ...
  case AxSCH::ICType::plain_wave: {
    ...
    if (SCH0 == PAR_ERR_VAL || SCH_k0 == PAR_ERR_VAL || Phase0 == PAR_ERR_VAL)
      Error("prob_initdata_state: in plain_wave IC---insufficient "
            "number of problem parameters!");
    // (geomdata.ProbHi(0) - geomdata.ProbLo(0)) is the comoving box size
    Real L_box_comov = geomdata.ProbHi(0) - geomdata.ProbLo(0);
    Real cur_time = 0;
    // Real a = Comoving::get_comoving_a(cur_time); // scale factor
    Real a = 1.0;
    Real L_box_phys = L_box_comov * a; // physical box size
                                       //
    Real k_wavenumber = SCH_k0 * 2.0 * M_PI /
                        L_box_phys; // convert mode number to // Needs to be 2pi
                                    // so we have a full period to // satisfy
                                    // smooth periodic boundary conditions.

    auto x_pos = [&](int iCell) {
      return geomdata.ProbLo(0) + iCell * geomdata.CellSize(0);
    };

    fields(i, j, k, AxSCH::getField(AxSCH::Fields::SCHf_Re)) =
        SCH0 * std::cos(k_wavenumber * x_pos(i) + Phase0);
    fields(i, j, k, AxSCH::getField(AxSCH::Fields::SCHf_Im)) =
        SCH0 * std::sin(k_wavenumber * x_pos(i) + Phase0);
  ...
  case AxSCH::ICType::linearperurbation: {
    Real del = 0.01;
    Real meandens =
        2.775e+11 * pow(Comoving::comoving_h, 2) * Comoving::comoving_OmM;
    ...
    fields(i, j, k, AxSCH::getField(AxSCH::Fields::SCHf_Re)) =
        std::sqrt((1.0 - del * cos(xtilde)) * meandens);
    fields(i, j, k, AxSCH::getField(AxSCH::Fields::SCHf_Im)) = 0.0;
    fields(i, j, k, AxSCH::getField(AxSCH::Fields::Dens)) =
        fields(i, j, k, AxSCH::getField(AxSCH::Fields::SCHf_Re)) *
            fields(i, j, k, AxSCH::getField(AxSCH::Fields::SCHf_Re)) +
        fields(i, j, k, AxSCH::getField(AxSCH::Fields::SCHf_Im)) *
    ...
    fields(i, j, k, AxSCH::getField(AxSCH::Fields::Phase)) =
        std::atan2(fields(i, j, k, AxSCH::getField(Fields::SCHf_Im)),
                   fields(i, j, k, AxSCH::getField(Fields::SCHf_Re)));
    break;
  } // end case linearperurbation

  default: {
    Error("prob_initdata_state: Selected initial condition type is not yet "
          "implemented!");
  } // end default
  } // end switch
} // end prob_initdata_pos
...
void AxSCH::prob_initdata_mom(
    ...
    Array4<amrex::GpuComplex<amrex::Real>> const
  ...
  switch (ic) {
    ...
    /*Error("prob_initdata_mom: in delta_k IC---not yet implemented!");*/
  default:
    Error("prob_initdata_mom: Selected initial condition type is not "
          "yet implemented!");
  }
}
#endif // BL_USE_MPI
