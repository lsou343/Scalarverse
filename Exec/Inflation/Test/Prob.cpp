...
void AxKG::prob_initdata_mom(
  ...
  case AxKG::ICType::standard:
    ...
    // variables to work
    {
      ...
      // Random initializers
      amrex::Real X =
          0.9; // rand_uniform();                  // amrex::Random();
      amrex::Real theta1 =
          2. * M_PI * 0.6; //* rand_uniform(); // amrex::Random();
      amrex::Real theta2 =
          2. * M_PI * 0.7; //* rand_uniform(); // amrex::Random();

      // LatticeEasy magnitude
      amrex::Real W2 = A * A * B * B * L * L * L /
                       (2. * omega * dx * dx * dx * dx * dx * dx);
      amrex::Real fMag = std::sqrt(-1. * W2 * std::log(X));
      if (i == debug_ix && j == debug_ix && k == debug_ix) {
        amrex::Print() << " --------- inputs for fR & fI --------- "
                       << std::endl;
      ...
      if (cutoff == 0 || i * i + j * j + k * k < cutoff * cutoff) {
        ...
        fdI = (invsqrt2 * (std::cos(theta2) - std::cos(theta1)) * fMag *
               omega); // ori
        if (i == debug_ix && j == debug_ix && k == debug_ix) {
          amrex::Print() << " --------- velocity before comov adding --------- "
                         << std::endl;
          amrex::Print() << "i = " << i << ", j = " << j << ", k = " << k
                         << std::endl;
          amrex::Print() << ", KGfv = " << fdR << " + i*" << fdI << "\n";
        }
  ...
  case AxKG::ICType::delta_k:
    ...
    // A simple k-space delta function. The real part of k_x == kG_k, k_y ==
    // k_z
