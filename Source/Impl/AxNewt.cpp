...
void AxNewt::initData() {
  ...
  std::cout << "AxNewt::initData: Checking state_new allocation..."
            << std::endl;
  if (state_new.nComp() <= 0 || state_new.boxArray().size() == 0) {
    amrex::Abort("AxNewt::initData: state_new not properly initialized.");
  } else {
    std::cout << "AxNewt::initData: state_new has " << state_new.nComp()
              << " components." << std::endl;
  }
  ...
  phigrav_new.setVal(0.); // LSR -- 0? Is this right?
...
void AxNewt::prob_initdata_pos(
  ...
  try {
    fields(i, j, k, AxNewt::getField(Fields::Density)) = density_value;
  } catch (const std::exception &e) {
...
void AxNewt::prob_initdata_mom(
    int i, int j, int k,
    const amrex::Array4<amrex::GpuComplex<amrex::Real>> &fields, // LSR -- this seems weird? Shouldn't it be const amrex:: if anything? Also what is it actually doing? 4 vector of some sort? Switched it around in any case
    const amrex::GeometryData &geomdata,
    const amrex::GpuArray<double, 10> &prob_param) {
  const Real *dx = geomdata.CellSize();
...
#ifdef INFLATION
  AxKGComov::variable_setup();  // LSR -- BaseAx -> AxKG
#else
  AxKG::variable_setup()
#endif
...
//  AxNewt::read_params(); // LSR -- why is this here? It's already covered by KG solver and gravity doesn't introduce any new parameters
...
int AxNewt::getField(Fields f) {
  ...
  case Fields::Density:
    ...
    break;
  case Fields::PhiGrav:
    field = 0;			// LSR -- some duplicate numbers, not an issue I think because it's for each descriptor
    break;
