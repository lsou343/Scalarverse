...
//    amrex::MultiFab&  density_old = get_level(level).get_old_data(AxNewt::getState(AxNewt::StateType::Density_Type));
...
//    amrex::MultiFab   Phi(KG_new.boxArray(), KG_new.DistributionMap(), 2, 1);

    //Phi_new.ParallelCopy(Phi_old, 0, 0, 1, 1, 1);
    // MultiFab::Copy(Phi_new, Phi_old,0,0,2,1);
...
//    KG.FillBoundary(geom.periodicity());
    ...
    gravity->solve_density_data(level, KG, density_new, invdeltasq, a, ap);
...
void AxNewt::kick_KG(amrex::Real time, amrex::Real dt_half, amrex::MultiFab&  mf_old, amrex::MultiFab&  mf_new, amrex::MultiFab&  Phi_old, const amrex::Real invdeltasq) // LSR -- Not made by me, but advances field derivative
{
    ...
    for (amrex::FillPatchIterator 
        ...
        fpi.isValid(); ++fpi)
//      for (amrex::MFIter mfi(mf_old, false); mfi.isValid(); ++mfi) {
        {
