...
amrex::Real compute_acceleration (amrex::Array4<amrex::Real> const& arr, amrex::Array4<amrex::Real> const& psi, int i, int j, int k, int comp, int compf, amrex::Real invdeltasq, amrex::Real a, amrex::Real ap, amrex::Real app) // Added psi array and compf
{
    ...
    else
    {
        ret = 
            ...
            //// - 4 psi' (f_pr' - r (a'/a) f_pr)
            + 4 * psi(i,j,k,compf) * (arr(i,j,k,compf) - r * (ap/a) * arr(i,j,k,comp)); // LSR -- Added terms - figure out what compf needs to be (field derivative) - 1, hard coded in atm
    }

    ret -= (1+2*psi(i,j,k,comp)) * compute_model_quantity({arr(i,j,k,comp)}, comp, a, ap, app, Quant::Vp); // LSR -- Addded psi
