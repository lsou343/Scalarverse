...
Gravity *BaseGrav::gravity = nullptr;

int BaseGrav::reuse_mlpoisson = 0;

BaseGrav::BaseGrav() { BL_PROFILE("BaseGrav::BaseGrav()"); }

BaseGrav::BaseGrav(amrex::Amr &papa, int lev, const amrex::Geometry &level_geom,
                   const amrex::BoxArray &ba,
                   const amrex::DistributionMapping &dm, amrex::Real time,
                   int dens, int stype, int ptype, int gtype)
    : AxSCHComov(papa, lev, level_geom, ba, dm, time) {
  BL_PROFILE("BaseGrav::BaseGrav(Amr)");
...
// void BaseGrav::build_metrics() {}

amrex::Real BaseGrav::get_comoving_a(amrex::Real time) const {
  return Comoving::get_comoving_a(time);
}

// Computes the volume-weighted sum of a grid quantity specified by its name at
// a given time.
Real BaseGrav::vol_weight_sum(const std::string &name, amrex::Real time,
                              bool masked) {
  BL_PROFILE("BaseGrav::vol_weight_sum(name)");

  // std::cout
  //     << "BaseGrav::vol_weight_sum: Starting calculation for derived quantity
  //     '"
...
// applying a mask.
Real BaseGrav::vol_weight_sum(const MultiFab &mf, bool masked) {
  BL_PROFILE("BaseGrav::vol_weight_sum");
