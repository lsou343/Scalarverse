...
void Gravity::solve_for_old_phi(int level, MultiFab &phi,
  ...
  MultiFab &density_mf =
      dynamic_cast<BaseGrav &>(*LevelData[level]).get_density(true);
...
void Gravity::solve_for_new_phi(int level, MultiFab &phi,
  ...
  // Retrieve the density field using get_density()
  MultiFab &density_mf =
      dynamic_cast<BaseGrav &>(*LevelData[level]).get_density();
  MultiFab::Copy(Rhs, density_mf, density, 0, 1, 0);
  // MultiFab::Add(Rhs, density_mf, density, 0, 1, 0);
...
void Gravity::solve_for_phi(int level, MultiFab &Rhs, MultiFab &phi,
  ...
  BaseGrav *cs = dynamic_cast<BaseGrav *>(
      &parent->getLevel(level)); // Get BaseGrav level pointer

  BL_ASSERT(cs != 0); // Ensure BaseGrav level pointer is valid
...
#pragma omp parallel if (!system::regtest_reduction)                           \
  ...
  Vector<Vector<std::unique_ptr<MultiFab>>> ec_gdPhi(fine_level - crse_level +
                                                     1);
  for (int lev = crse_level; lev <= fine_level; lev++) {
    BaseGrav *BaseGrav_lev = dynamic_cast<BaseGrav *>(&parent->getLevel(lev));
    // AmrLevel* BaseGrav_lev = dynamic_cast<AmrLevel*>(&parent->getLevel(lev));
    // // These should only need the AmrLevel bits -PH
    ec_gdPhi[lev - crse_level].resize(AMREX_SPACEDIM);
    for (int n = 0; n < AMREX_SPACEDIM; ++n)
      ec_gdPhi[lev - crse_level][n].reset(
          new MultiFab(BaseGrav_lev->getEdgeBoxArray(n),
                       BaseGrav_lev->DistributionMap(), 1, 0));
  }
...
void Gravity::get_crse_grad_phi(
  ...
  // BaseGrav* BaseGrav_crse_lev =
  // dynamic_cast<BaseGrav*>(&parent->getLevel(level-1)); -PH
  AmrLevel *BaseGrav_crse_lev =
  ...
  for (int i = 0; i < AMREX_SPACEDIM; ++i) {
    ...
    grad_phi_crse[i].reset(new MultiFab(BaseGrav_crse_lev->getEdgeBoxArray(i),
                                        BaseGrav_crse_lev->DistributionMap(), 1,
                                        0));
...
#endif
  BaseGrav *cs = dynamic_cast<BaseGrav *>(&parent->getLevel(level));
  ...
  for (int lev = 0; lev < num_levels; lev++) {
    ...
    if (is_new == 1) {
      MultiFab::Copy(
          // MultiFab::Add(
          *Rhs_p[lev],
          dynamic_cast<BaseGrav &>(*LevelData[level + lev]).get_density(),
          density, 0, 1, 0);
    } else if (is_new == 0) {
      MultiFab::Copy(
          // MultiFab::Add(
          *Rhs_p[lev],
          dynamic_cast<BaseGrav &>(*LevelData[level + lev]).get_density(true),
          density, 0, 1, 0);
    }
  }
  ...
  if (parent->Geom(level).isAllPeriodic()) {
    ...
    if (verbose > 1)
      ...
      if (parent->Geom(level).Domain().numPts() == grids[level].numPts()) {
        Real sum = 0;
        for (int lev = 0; lev < num_levels; lev++) {
          BaseGrav *BaseGrav_level =
              dynamic_cast<BaseGrav *>(&(parent->getLevel(level + lev)));
          sum += BaseGrav_level->vol_weight_sum(*Rhs_p[lev], true);
        }
...
void Gravity::set_mass_offset(amrex::Real time) {
  ...
  if (geom.isAllPeriodic()) {
    ...
    for (int lev = 0; lev <= flev; lev++) {
      BaseGrav *cs = dynamic_cast<BaseGrav *>(&parent->getLevel(lev));

      BL_ASSERT(cs != 0);

      // generalized function
      mass_offset += cs->vol_weight_sum("Dens", time, true);
    }
...
void Gravity::AddParticlesToRhs(int level, MultiFab &Rhs, int ngrow) {
  ...
  for (int i = 0; i < BaseGrav::theActiveParticles().size(); i++) {
    BaseGrav::theActiveParticles()[i]->AssignDensitySingleLevel(particle_mf,
                                                                level);
    amrex::Gpu::Device::streamSynchronize();
...
void Gravity::AddParticlesToRhs(int base_level, int finest_level, int ngrow,
  ...
  for (int i = 0; i < BaseGrav::theActiveParticles().size(); i++) {
    ...
    BaseGrav::theActiveParticles()[i]->AssignDensity(PartMF, base_level, 1,
                                                     finest_level, ngrow);
#ifdef AMREX_DEBUG
    for (int lev = 0; lev < num_levels; lev++) {
...
void Gravity::AddVirtualParticlesToRhs(int level, MultiFab &Rhs, int ngrow) {
  ...
  if (level < parent->finestLevel()) {
    ...
    for (int i = 0; i < BaseGrav::theVirtualParticles().size(); i++) {
      particle_mf.setVal(0.);
      BaseGrav::theVirtualParticles()[i]->AssignDensitySingleLevel(particle_mf,
                                                                   level, 1, 1);
      MultiFab::Add(Rhs, particle_mf, 0, 0, 1, 0);
    }
  }
...
void Gravity::AddVirtualParticlesToRhs(
  ...
  if (finest_level < parent->finestLevel()) {
    ...
    for (int i = 0; i < BaseGrav::theGhostParticles().size(); i++) {
      BaseGrav::theVirtualParticles()[i]->AssignDensitySingleLevel(
...
void Gravity::AddGhostParticlesToRhs(int level, MultiFab &Rhs) {
  ...
  if (level > 0) {
    ...
    for (int i = 0; i < BaseGrav::theGhostParticles().size(); i++) {
      ghost_mf.setVal(0.);
      BaseGrav::theGhostParticles()[i]->AssignDensitySingleLevel(
...
void Gravity::CorrectRhsUsingOffset(int level, MultiFab &Rhs) {
  ...
  if (level == 0) {
    BaseGrav *BaseGrav_level = dynamic_cast<BaseGrav *>(&(parent->getLevel(0)));
    Real sum = BaseGrav_level->vol_weight_sum(Rhs, false);
