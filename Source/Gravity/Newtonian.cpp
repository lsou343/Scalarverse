...
Gravity::Gravity(Amr *Parent, int /*_finest_level*/, BCRec *_phys_bc,
                 int _density, int density_comp, int pstate_comp)
      ...
      phys_bc(_phys_bc) // Initialize physical boundary conditions
{
  ...
  Density_Type = density_comp; // Assuming these are the correct values
...
//    pp.query("calc_steps", calc_steps);
...
void Gravity::solve_for_phi(int level, MultiFab &Rhs, MultiFab &phi,
  ...
  // comoving scale factor -PH // LSR -- isn't this wrong for density but correct for field?
...
void Gravity::get_old_grav_vector(int level, MultiFab &grav_vector, Real time) {
  ...
  AmrLevel::FillPatch(*amrlev, grav_vector, ng, time, PhiGravv_Type, 0,
                      AMREX_SPACEDIM); // Fill ghost cells
}

// LSR -- TODO: Get new PhiGravv vector
void Gravity::get_new_grav_vector(int level, MultiFab &grav_vector, Real time) {
  ...
  MultiFab &G_new = LevelData[level]->get_new_data(PhiGravv_Type);
